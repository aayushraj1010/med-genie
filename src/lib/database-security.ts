...
export class DatabaseSecurity {
  ...
  static validateEmail(email: string): boolean {
    ...
    if (!isValidFormat || !isValidLength || !isNotTooShort) return false;

    // Reject common typo domains (simple blacklist)
    try {
  ...
  static validateName(name: string): boolean {
    ...
    const isValidLength =
      sanitized.length >= 2 && sanitized.length <= this.MAX_NAME_LENGTH;
  ...
  static logDatabaseAccess(auditLog: DatabaseAuditLog): void {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        `[DB AUDIT] ${timestamp} - User: ${auditLog.userId || "anonymous"}, Action: ${auditLog.action}, Table: ${auditLog.table}`,
      );
