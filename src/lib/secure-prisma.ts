...
class SecurePrismaClient extends PrismaClient {
  ...
  async findUserByEmail(email: string, ipAddress?: string) {
    ...
    // Check rate limiting
    if (
      !DatabaseSecurity.checkRateLimit("findUserByEmail", undefined, 20, 60000)
    ) {
  ...
  // Find user by name (case-insensitive) to enforce username uniqueness
  async findUserByName(name: string, ipAddress?: string) {
    if (!name || typeof name !== "string") {
      throw new Error("Invalid name");
    }

    // Sanitize name
    const sanitized = DatabaseSecurity.sanitizeInput(name, 100);

    DatabaseSecurity.logDatabaseAccess({
      userId: undefined,
      action: "FIND_USER_BY_NAME",
      table: "user",
      details: `Name lookup: ${sanitized}`,
      ipAddress,
      success: true,
    });

    return this.user.findFirst({
      where: { name: { equals: sanitized, mode: "insensitive" } },
  ...
  async createUser(userData: any, ipAddress?: string) {
    ...
    // Check rate limiting
    if (!DatabaseSecurity.checkRateLimit("createUser", undefined, 5, 3600000)) {
      // 5 per hour
  ...
  async updateUser(userId: number, updateData: any, ipAddress?: string) {
    ...
    // Check rate limiting
    if (!DatabaseSecurity.checkRateLimit("updateUser", userId, 50, 60000)) {
      // 50 per minute
  ...
  async deleteUser(userId: number, ipAddress?: string) {
    ...
    // Check rate limiting
    if (!DatabaseSecurity.checkRateLimit("deleteUser", userId, 3, 3600000)) {
      // 3 per hour
  ...
  async getUserProfile(userId: number, ipAddress?: string) {
    ...
    // Check rate limiting
    if (
      !DatabaseSecurity.checkRateLimit("getUserProfile", userId, 100, 60000)
    ) {
      // 100 per minute
  ...
  async searchUsers(searchTerm: string, ipAddress?: string) {
    ...
    // Check rate limiting
    if (!DatabaseSecurity.checkRateLimit("searchUsers", undefined, 30, 60000)) {
      // 30 per minute
